...
// Função que pode ser chamada via HTTP ou cron
serve(async (req) => {
  if (req.method === 'OPTIONS') {
    return new Response(null, { headers: corsHeaders });
  }

  // Verificar se é uma chamada de cron (com header especial)
  ...
  try {
    ...
    console.log('Starting automatic payment polling check...');
    ...
    for (const payment of pendingPayments) {
      try {
        ...
        if (paymentData.status === 'approved') {
          ...
          await supabase.from('logs').insert({
            poltrona_id: payment.poltrona_id,
            message: `Pagamento ${payment.payment_id} ${paymentData.status} - detectado por polling automático`
          });
        }
    ...
    console.log(`Automatic polling complete: ${approvedCount} approved, ${rejectedCount} rejected`);
    ...
    console.error('Error in payment-polling-cron:', error);
